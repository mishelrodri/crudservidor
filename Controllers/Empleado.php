	...
	class Empleado extends Controllers{
		...
		public function setEmpleado(){
			...
			if($intIdEmpleado == 0)
			{
				...
				//Crear
				$idinsertar = $this->model->getNewId();
				$request_Empleado = $this->model->insertEmpleado($idinsertar,$strDui,$strNombre,$strApellido,$strDireccion,$intestado,$intCargo);
				//}
			}else{
				$option = 2;
				//if ($_SESSION['permisosMod']['actualizar']) {
				//Actualizar
				$request_Empleado = $this->model->updateEmpleado($intIdEmpleado, $strDui,$strNombre,$strApellido,$strDireccion,$intestado,$intCargo);
				//}
			}
